...
const Input = React.memo(({ label, error, required, className = "", ...props }) => {
	return (
...
// Step components are defined outside to keep their identity stable between renders
const PersonalStep = React.memo(function PersonalStep({ form, update, markTouched, touched, stepErrors }) {
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 gap-3 animate-slide-up">
			<Input 
				...
				error={touched.firstName && stepErrors.firstName} 
				required 
			/>
			<Input 
				label="Last name" 
				...
				error={touched.lastName && stepErrors.lastName} 
				required 
			/>
			<Input 
				label="Email" 
				...
				error={touched.email && stepErrors.email} 
				required 
			/>
			<Input 
				label="Phone" 
				...
				error={touched.phone && stepErrors.phone} 
				required 
			/>
			<Input 
				label="Date of birth" 
				...
				error={touched.dateOfBirth && stepErrors.dateOfBirth} 
				required 
			/>
			<Select 
				label="Gender" 
				...
				error={touched.gender && stepErrors.gender} 
				required
			>
			...
			<Input 
				...
				error={touched.address_line1 && stepErrors.address_line1} 
			...
			<Input 
				...
				error={touched.address_city && stepErrors.address_city} 
				required 
			/>
			<Input 
				label="State" 
				...
				onBlur={() => markTouched("address_state")} 
				error={touched.address_state && stepErrors.address_state} 
				required 
			/>
			<Input 
				label="Postal Code" 
				...
				onBlur={() => markTouched("address_postalCode")} 
				error={touched.address_postalCode && stepErrors.address_postalCode} 
				required 
			/>
			<Input 
				label="Country" 
				...
				error={touched.address_country && stepErrors.address_country} 
				required 
			/>
		</div>
	)
...
const EmploymentStep = React.memo(function EmploymentStep({ form, update, markTouched, touched, stepErrors, departments, designations }) {
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 gap-3 animate-slide-up">
			<Select
				label="Designation"
				...
				error={touched.designationId && stepErrors.designationId}
				required
			...
			<Select
				...
				error={touched.departmentId && stepErrors.departmentId}
				required
				className="text-gray-900"
			>
				<option value="">Select…</option>
			...
			<Select
				...
				error={touched.managerId && stepErrors.managerId}
				required
				className="text-gray-900"
			>
				<option value="">Select…</option>
			...
			<Input
				...
				error={touched.joiningDate && stepErrors.joiningDate}
				required
			/>
			<Input
			...
			<Input
				...
				error={touched.salary && stepErrors.salary}
				required
			/>
		</div>
	)
...
const EmergencyStep = React.memo(function EmergencyStep({ form, update, markTouched, touched, stepErrors }) {
	return (
		<div className="grid grid-cols-1 md:grid-cols-2 gap-3 animate-slide-up">
			<Input
				...
				error={touched.ec_name && stepErrors.ec_name}
				required
			/>
			<Input
				...
				error={touched.ec_phone && stepErrors.ec_phone}
				required
			/>
			<Input
				label="Relationship"
				...
				error={touched.ec_relation && stepErrors.ec_relation}
				required
			/>
			<Input
				...
				error={touched.ec_email && stepErrors.ec_email}
				required
				className="md:col-span-2"
			/>
			<Select
				...
				error={touched.language && stepErrors.language}
				required
				className="text-gray-900"
			>
				<option value="">Select…</option>
			...
			<Select
				...
				error={touched.timezone && stepErrors.timezone}
				required
				className="text-gray-900"
			>
				<option value="">Select…</option>
...
const ReviewStep = React.memo(function ReviewStep() {
	return (
		<div className="animate-scale-in">
			...
			{/* ... Review step content remains the same ... */}
		</div>
	)
})
...
export default function SmartEmployeeOnboardingForm({ initialData = {}, onSubmit, className = "" }) {
	...
	// Memoize update function to prevent recreation
	...
	// Memoize markTouched to prevent recreation
	const markTouched = React.useCallback((key) => {
		setTouched((t) => ({ ...t, [key]: true }))
		// Only validate this specific field when it's touched
		const errors = getStepErrors(step, form)
		if (errors[key]) {
			setStepErrors(prev => ({ ...prev, [key]: errors[key] }))
		} else {
			setStepErrors(prev => {
