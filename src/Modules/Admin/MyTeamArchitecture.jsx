...
export default function MyTeamArchitecture() {
  ...
  return (
    <div className="team-container flex h-[calc(100vh-80px)] overflow-hidden">
      <SmartToster message={toastMsg} duration={2500} onClose={() => setToastMsg("")} />
      {/* Left: Team members list (no designations) */}
      <aside className="team-sidebar relative w-72 md:w-80 border-r border-orange-500/20 dark:border-orange-500/40 flex flex-col">
        <header className="team-sidebar sticky top-0 z-10 border-b border-orange-500/20 dark:border-orange-500/40 px-3 py-3">
          ...
          <div className="mt-2 relative">
            <input
              className="w-full rounded-lg border border-orange-500/30 dark:border-orange-500/50 bg-white dark:bg-black px-8 py-1.5 text-sm text-neutral-900 dark:text-white outline-none focus:ring-2 focus:ring-orange-500/50"
              placeholder="Search name or email…"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
            />
            <Search size={14} className="absolute left-2 top-1/2 -translate-y-1/2 text-orange-500 dark:text-orange-400" />
          </div>
        </header>
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1.5 scrollbar-thin scrollbar-thumb-orange-400 hover:scrollbar-thumb-orange-500">
          {flowLoading === "loading" && (
            <div className="text-xs text-neutral-600 dark:text-neutral-400">Initializing…</div>
          )}
          {flowError && (
          ...
          {usersList
            ...
            .map((u) => (
              <div key={u.id} className="team-card group border border-orange-500/30 dark:border-orange-500/50 rounded-xl shadow-sm hover:shadow-md hover:shadow-orange-500/20 transition-all duration-200 p-2.5">
                <div className="flex items-center gap-3">
                  <Avatar src={toAssetUrl(u.avatar)} name={u.name} />
                  <div className="min-w-0">
                    <div className="team-text-primary text-[13px] font-semibold truncate">{u.name}</div>
                    <div className="team-text-secondary text-[11px] truncate flex items-center gap-1">
                      <Mail size={12} className="text-orange-500 dark:text-orange-400" /> {u.email}
                    </div>
                  </div>
                </div>
                <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                  <span className="designation-badge-level text-[10px] px-1.5 py-0.5 rounded border">L{u.level ?? "-"}</span>
                  <span className="designation-badge-route text-[10px] px-1.5 py-0.5 rounded border inline-flex items-center gap-1">
        ...
        <footer className="team-sidebar sticky bottom-0 z-10 border-t border-orange-500/20 dark:border-orange-500/40 px-3 py-2 flex items-center justify-between text-[11px] text-neutral-600 dark:text-neutral-400">
          <div className="flex items-center gap-1.5">
            <Clock size={12} className="text-orange-500 dark:text-orange-400" />
            <span>Updated just now</span>
          </div>
          <button className="text-orange-600 dark:text-orange-400 hover:text-orange-800 dark:hover:text-orange-300 font-medium" onClick={() => dispatch(fetchDesignationsFlow())}>
            Refresh
          </button>
        </footer>
      </aside>
          ...
          {!selected ? (
            <div className="h-full flex items-center justify-center">
              <div className="text-center max-w-md mx-auto">
                <div className="mx-auto w-12 h-12 rounded-2xl bg-orange-100 dark:bg-orange-900/30 border border-orange-200 dark:border-orange-500/50 flex items-center justify-center text-orange-700 dark:text-orange-400 mb-3">
                  <Users size={20} />
                </div>
                <h2 className="team-text-primary text-lg font-semibold">Loading user architecture…</h2>
                <p className="team-text-secondary text-sm mt-1">We'll fetch members and render their structure once data is ready.</p>
              </div>
            </div>
          ) : (
            <div className="border border-orange-500/30 dark:border-orange-500/50 rounded-2xl overflow-hidden bg-white/80 dark:bg-black/80 backdrop-blur-sm">
              <div className="team-sidebar px-4 py-3 border-b border-orange-500/20 dark:border-orange-500/40 flex items-center justify-between">
                <div className="flex items-center gap-2 team-text-primary">
              ...
              <div className="team-canvas h-[calc(100vh-200px)] rounded-xl overflow-hidden">
                {loading ? (
                  <div className="h-full w-full grid place-items-center text-neutral-600 dark:text-neutral-400">Loading flow…</div>
                ) : error ? (
...
function UserNode({ data }) {
  ...
  return (
    <div className="relative w-[220px] group">
      <Handle type="target" position={Position.Top} className="!w-2 !h-2 !bg-orange-500 dark:!bg-orange-400 !border !border-orange-600 dark:!border-orange-500" />
      <div className="team-user-node relative z-20 rounded-xl border border-orange-500/30 dark:border-orange-500/40 px-3 py-2 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex items-start gap-2">
          <Avatar src={toAssetUrl(avatar)} name={name} />
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="team-text-primary text-[12px] font-semibold truncate">{name}</div>
                <div className="team-text-secondary text-[10px] truncate flex items-center gap-1"><Mail size={12} className="text-orange-500 dark:text-orange-400" /> {email}</div>
              </div>
              <div className="text-right shrink-0">
                <div className="designation-badge-route text-[10px] px-1.5 py-0.5 rounded-full inline-block">{designationTitle || "—"}</div>
                <div className="team-text-secondary text-[9px] mt-1">L{level ?? "-"} • <Briefcase size={10} className="inline mr-0.5" /> {projectsCount ?? 0}</div>
              </div>
            </div>
            <div className="mt-1 flex items-center gap-1.5 flex-wrap">
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 text-neutral-700 dark:text-neutral-300 inline-flex items-center gap-1">
                <ListChecks size={12} /> {tasks?.totalAssigned ?? 0}
              </span>
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-500/50 text-green-700 dark:text-green-400 inline-flex items-center gap-1">
            ...
            {Array.isArray(projects) && projects.length > 0 && (
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {projects.slice(0, 2).map((p) => (
                  <span key={p.id} className="designation-badge-level text-[10px] px-1.5 py-0.5 rounded inline-flex items-center gap-1">
                    <Briefcase size={12} /> {p.name}
                  </span>
                ))}
                {projects.length > 2 && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 text-neutral-700 dark:text-neutral-300">+{projects.length - 2} more</span>
                )}
              </div>
            )}
            {/* Tasks by status */}
      ...
      <Handle type="source" position={Position.Bottom} className="!w-2 !h-2 !bg-orange-500 dark:!bg-orange-400 !border !border-orange-600 dark:!border-orange-500" />
    </div>
  );
}
// Avatar helper
  ...
  if (src) {
    return (
      <img src={src} alt={name} className="h-8 w-8 rounded-full object-cover border border-orange-500/30 dark:border-orange-500/50" />
    );
  }
  return (
    <div className="h-8 w-8 rounded-full bg-gradient-to-br from-orange-500 to-orange-900 dark:from-orange-400 dark:to-orange-800 text-white flex items-center justify-center text-[11px] font-semibold shadow-sm">
...
function Chip({ icon, label }) {
  return (
    <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-neutral-100 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 text-neutral-700 dark:text-neutral-300">
      {icon} {label}
    </span>
  );
}
