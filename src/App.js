...
import UserCreateProject from "./Modules/User/UserCreateProject";
import UserProjectList from "./Modules/User/UserProjectList";
...
function PublicAuthLayout({ children }) {
  return (
    <div className="auth-public-root flex items-center justify-center p-6">
      {children}
    </div>
  );
}
...
function AppRoutes() {
  ...
  return (
    <Routes>
      ...
      <Route
        path="/uas/portal/auth/login"
        element={
          <PublicAuthLayout>
            <SmartChallangeAuthForm
              passwordOnly
              initialEmail={challenge.email || ""}
      ...
      <Route element={<ProtectedRoute roles={[ROLES.SUPER_ADMIN, ROLES.IT]} />}>
        <Route
          path=":platformId/supeadmin/:userId"
          element={<SuperAdminPlatformLayout />}
        >
