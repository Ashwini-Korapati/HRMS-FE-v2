...
import { createSlice, createAsyncThunk, nanoid } from '@reduxjs/toolkit'
import { httpPostService, httpGetService, httpDeleteService, httpPutService } from '../../config/httphandler'
import { selectAuthUser, selectAuthCompany } from './authSlice'
...
const initialState = {
  ...
  lastCreated: null,
  version: nanoid(6),
  currentEditing: null // Track which department is being edited
}
    ...
    builder
      ...
      .addCase(updateDepartment.fulfilled, (state, action) => {
        ...
        if (dept && dept.id) {
          const idx = state.items.findIndex(d => d.id === dept.id)
          if (idx >= 0) {
            state.items[idx] = { ...state.items[idx], ...dept }
          }
        }
        state.currentEditing = null
      })
...
export const selectDepartmentDeleting = (s) => s.departments.deleting
export const selectDepartmentDeleteError = (s) => s.departments.deleteError
export const selectCurrentEditingDepartment = (s) => s.departments.currentEditing
export default departmentSlice.reducer